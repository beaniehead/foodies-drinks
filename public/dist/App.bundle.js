...
function loadPlaces(map) {
  var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : mapOptions.center.lat;
  var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : mapOptions.center.lng;

  // function loadPlaces(map, { lat, lng } = mapOptions.center) {
  _axios2.default.get("/api/stores/near?lat=" + lat + "&lng=" + lng).then(function (res) {
    var places = res.data;
    if (!places.length) {
      var inner = (0, _bling.$)(".inner__map");
      var mapNode = (0, _bling.$)(".map");
      var warnDivClass = (0, _bling.$)(".warning");
      if (!warnDivClass) {
        ...
        var warnText = document.createTextNode("No places Found!");
    ...
    _axios2.default.get("/api/search?q=" + this.value).then(function (res) {
      if (res.data.length) {
        ...
        items.forEach(function (item) {
          return item.addEventListener("mouseover", function () {
            items.forEach(function (itemOver) {
              return itemOver.classList.remove("search__result--active");
            });
...
var _confirmDelete = __webpack_require__(11);
